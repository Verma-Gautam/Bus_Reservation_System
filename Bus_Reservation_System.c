...
#include <stdlib.h>
#include <string.h>
char ch[10][130] = {"Rohtak Express", "Ambala Liner", "Panchkula Express", "Karnal Bus Lines", "Panipat Express", "Gurugram SpaceX"};
char name[32][100] = {'\0'};
char number[32][2] = {'\0'};
int num1[32] = {0};
int trno;
    ...
    do
    {
        ...
        switch (num)
        {
	        ...
	        case 2:
	            ...
	            break;
	        case 3:
	            cancle();  // to cancel tickets
	            break;
	        case 4:
	            status();  // to find the status of buses
	            break;
	        case 5:
	        	printf("  HERE WE GO");
	        	break;
...
void booking()
{
    ...
    else
    {
        ...
        seat1 = seat1 - booking;
        itoa(trno, numstr, 10); // converting trno(bus no. into string(numstr) with base 10
		name_number(booking, numstr); // numstr contains the bus number for which the function is called
...
void name_number(int booking, char numstr[100])
{
    ...
    FILE *a, *b;
    int i = 0;
    strcat(numstr, ".txt"); // it will became like 1.txt or 2.txt and so on...
    strcat(tempstr1, numstr); // it will be status1.txt or status2.txt and so on...
    strcat(tempstr2, numstr); // here it became number1.txt or number2.txt and so on...
    
    // we have files named similar to status1.txt and number1.txt
    ...
    for (i = 0; i < booking; i++) // for entering the person name and seat number in the file
    {
        ...
        scanf("%s", name[number - 1]); // getting the name of the person from user
        printf("\n======================================================================================================\n\n");
        printf("\n");
        itoa(number, tempstr, 10); // converting the seatnumber into string(tempstr)
        fprintf(a, "%s ", name[number - 1]); // putting the name of the person in the bus file at corresponding(seat) seat number
        fprintf(b, "%s ", tempstr); // and putting the respective seat number in the (number) file
    }
    fclose(a);
    fclose(b);
}
